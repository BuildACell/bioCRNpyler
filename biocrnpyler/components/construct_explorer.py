...
class ConstructExplorer(LocalComponentEnumerator):
    def __init__(
        ...
        name,
        direction='forward',
        possible_directions=('forward', 'reverse'),
    ):
...
class TxExplorer(ConstructExplorer):
    ...
    def iterate_part(self, part, reading_direction):
        ...
        # Case 2: Terminator
        elif effective_direction == 'forward' and isinstance(
            part, Terminator
        ):  # Kill Sarah Day O'Connor
            self.terminate_loop()
    ...
    def return_components(self, component, previously_enumerated=None):
        ...
        for rna in self.all_rnas.values():
            if (previously_enumerated is None) or (
                rna not in previously_enumerated
            ):
                return_rnas += [rna]
            else:
                pass
...
class TlExplorer(ConstructExplorer):
    ...
    def iterate_part(self, part, reading_direction):
        ...
        # Case 2: CDSes in the correct orientation
            ...
            rbses_translating > 0
            and isinstance(part, CDS)
            and effective_direction == 'forward'
        ):
