...
class Construct(Component, OrderedPolymer):
    ...
    **kwargs
    ...
    def enumerate_components(self, previously_enumerated=None):
        ...
        **1. Binding-induced species**

        When a component creates a species that binds to part of the
        construct. For example, `<A,B,C>` --> `<[A:RNAP],B,C>` would return
        component A configured for this binding.

        **2. Combinatorial binding states**

        When multiple components can bind simultaneously. For construct
        `<A,B,C>` where both A and B can bind RNAP:
