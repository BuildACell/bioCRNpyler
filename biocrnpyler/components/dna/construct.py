...
class Construct(Component, OrderedPolymer):
    def __init__(
            self, parts_list, name=None, circular=False,
            mechanisms=None,  # custom mechanisms
            parameters=None,  # customized parameters
            attributes=None, initial_concentration=None,
            component_enumerators=None, make_dirless_hash=True,
            **kwargs):
    ...
    def get_circularly_permuted(self, new_first_position):
        ...
        if not self.circular:
            ...
            return DNA_construct(
                ...
                self.parts_list[:new_first_position], circular=True,
                component_enumerators = self.component_enumerators,
                attributes=self.attributes,mechanisms=self.mechanisms,
    ...
    def update_permutation_hash(self):
        """update the unique string generated to represent the content
        of this dna construct regardless of orientation and rotation"""
    ...
    def update_base_species(self, base_name=None, attributes = None):
        if base_name is None:
            self.base_species = self.set_species(
                self.name, material_type=self.material_type,
                attributes=attributes)
        else:
            self.base_species = self.set_species(
                base_name, material_type=self.material_type,
                attributes=attributes)
    ...
    def add_mechanism(
        ...
        Component.add_mechanism(
            self, mechanism, mech_type = mech_type, overwrite=overwrite,
            optional_mechanism = optional_mechanism)
        for part in self.parts_list:
            part.add_mechanism(
                mechanism, mech_type=mech_type, overwrite=overwrite,
                optional_mechanism = optional_mechanism)
    ...
    def __contains__(self,obj2):
        """checks if this construct contains a certain part, or a copy
        of a certain part"""
    ...
    def get_species(self):
        """returns the species of this dna construct, using
        OrderedPolymerSpecies"""
        ocomplx = []
        for part in self.parts_list:
    ...
    def located_allcomb(self,spec_list):
        ...
        def recursive_path(in_list):
            """this function takes every possible "path" through a list of
            lists.
    ...
    def get_partlist_hash(cls,partlist):
        """creates a string containing the name and direction of all parts
        in a list of parts (but not their position)"""
        partlist_str = '_'.join(
            [str(a[0]) + str(b) for a, b in zip(partlist,range(len(partlist)))])
        return partlist_str

    @classmethod
    def create_hashless_reverse(cls,construct):
        """create a reverse construct but don't calculate its hash (because
        that would make an infinite loop)"""
        rev_con = [a.get_orphan() for a in construct]
        for rev_part,origpart in zip(rev_con,construct):
            rev_part.direction = {
                'forward': 'reverse', 'reverse': 'forward'}[origpart.direction]
        rev_con = rev_con[::-1]
        rev_con = Construct(
    ...
    def rotation_free_hash(cls,construct):
        ...
        alphabetically ordered. Every part is considered as a potential
        starting point, and the most alphabetically ordered order is then
        chosen as the best permutation
        ...
        for part in construct:
            ...
            if best_partlist[0][0] is None:
                ...
                while test_partlist[-1][0] == \
                    ...
                    test_partlist += [
                        (Construct.get_partstring(next_testpart),next_testpart)]
        ...
        return Construct.get_partlist_hash(
            best_partlist),1,best_partlist[0][1].position
    @classmethod
    def direction_rotation_free_hash(cls,construct):
        """Computes the best circular permutation of a construct, forward
        and reverse. Then returns whichever one of those comes
    ...
    def linear_direction_free_hash(cls,construct):
        """creates a string representing the construct forward or reverse,
        and returns whichever of those is alphabetically first
    ...
    def omnihash(cls,construct):
        """A construct can exist forwards or backwards, and circularly
        permuted (but only if it's a circular construct).
...
class DNA_construct(Construct,DNA):
    def __init__(
            ...
            mechanisms=None,  # custom mechanisms
            parameters=None,  # customized parameters
            attributes=None, initial_concentration=None, copy_parts=True,
            component_enumerators = None, **kwargs):
        ...
        Construct.__init__(
            self=self, parts_list =parts_list, name = name, circular=circular,
            mechanisms=mechanisms, parameters=parameters,
            attributes=attributes, initial_concentration=initial_concentration,
            component_enumerators = component_enumerators, **kwargs)
...
class RNA_construct(Construct,RNA):
    def __init__(
            self, parts_list, name=None, promoter=None,
            component_enumerators=None, length=0, **kwargs):
        ...
        Construct.__init__(
            self, parts_list=parts_list, circular=False, name=name,
            component_enumerators=component_enumerators, **kwargs)
...
class DNA_part(Component, OrderedMonomer):
    def __init__(
            self, name, assembly=None, direction=None, pos=None,
            sequence=None, no_stop_codons=[], material_type='part', **kwargs):
            ...
            if self.name == other.name:
                if self.assembly is not None and other.assembly is not None:
                    ...
                    return False
                     ...
                     (self.parent is not None and other.parent is not None \
