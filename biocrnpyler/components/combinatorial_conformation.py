...
class CombinatorialConformation(Component):
    ...
    def __init__(
            self, final_states, initial_states=None, intermediate_states=None,
            excluded_states=None, state_part_ids=None, name=None, **kwargs):
    ...
    def _assert_conformation(self, states, input_name = "states"):
        ...
        if not all([str(p) == str(self.internal_polymer)
                    for s in states for p in s.polymers]):
            raise ValueError(
                f"All PolymerConformations in {input_name} must contain "
                f"a single unique internal OrderedPolymerSpecies. "
                f"Recieved: {states}.")
    ...
    def compute_species_changes(self, s0, sf):
        ...
        if any([len(s0.get_complexes_at(0, i)) >
            ...
            for cf in sf.complexes:
                ...
                for ind in pf_inds:
                    ...
                    for c0 in found_complexes:
                        ...
                        if (cf, pf_inds) not in merged_complexes or \
                           c0 not in merged_complexes[cf, pf_inds]:
                ...
                sub_complex_exclusive_species = [
                    i for i in sub_complex_species_and_inds_str
                    if i not in cf_species_and_inds_str]
                cf_complex_exclusive_species = [
                    i for i in cf_species_and_inds_str
                ...
                # produce the new complex
                    ...
                    species_changes[cf, pf_inds] = [
                        ...
                        if (cf_species_and_inds_str[i] not in
                            sub_complex_species_and_inds_str
                            and cf_species_and_inds[i][1] is None)]
        # If any complex cannot be created, return False
        if species_changes == False or any(
                [species_changes[k] is False for k in species_changes]) or \
                (len(species_changes) == 0 and len(merged_complexes) == 0):
    ...
    def get_combinations_between(self, s0, sf):
        ...
        if X is False:
            ...
            for cf in sf.complexes:
                ...
                if ((cf, inds) in merged_complexes and
                    len(merged_complexes[(cf, inds)]) == 1 and
                    merged_complexes[(cf, inds)][0].monomer_eq(cf)) \
                    or (cf, inds) not in merged_complexes:
                    complexes_to_merge = None
                else:
                    complexes_to_merge = merged_complexes.get((cf, inds))

                if (cf, inds) not in species_changes or \
                   len(species_changes[cf, inds]) == 0:
                    species_to_add = None
                else:
                ...
                #     (cf, inds, species_to_add, complexes_to_merge))
                if species_to_add is not None or \
                   complexes_to_merge is not None:
                    changes_list.append(
                        (cf, inds, species_to_add, complexes_to_merge))
            ...
            for perm in perms:
                ...
                for cf, inds, species_to_add, complexes_to_merge in perm:
                    ...
                    if old_state not in self.excluded_states and \
                       new_state not in self.excluded_states:
                        combinations.append(
            ...
            for s0 in self.initial_states:
                for si in self.intermediate_states:
                    if si != s0:
                        ...
                        # between s0 and si
                        for (s00, additional_species, sff) in \
                                self.combination_dict[s0, si]:
                            part_id = f"{self._get_part_id(s00)}" + \
...
class CombinatorialConformationPromoter(CombinatorialConformation, Promoter):
    ...
    """
    def __init__(
            self, promoter_states, promoter_location, promoter_states_on=True,
            activating_complexes=None, inactivating_complexes=None,
            intermediate_states=None, final_states=None,
            name='CombinatorialConformationPromoter', **kwargs):

        Promoter.__init__(self, name, **kwargs)
        CombinatorialConformation.__init__(
            self, name=name, intermediate_states=intermediate_states,
            final_states=final_states, **kwargs)
        ...
        assert all([
            isinstance(c, ComplexSpecies) for c in self.inactivating_complexes])
    ...
    def update_species(self):
        ...
        for s in self.conformation_species:
            if isinstance(s, PolymerConformation):
                active_state = False
                if  (s in self.promoter_states and self.promoter_states_on) or \
                    (s not in self.promoter_states and
                     not self.promoter_states_on):
                    active_state = True

                active_complex = False
                if any([s.get_complex(c) is not None
                ...
                if any([s.get_complex(c) is not None
                    ...
                    if (active_state and len(self.promoter_states) > 0) or \
                       active_complex:
                        warnings.warn(
    ...
    def update_reactions(self):
        ...
        conformation_reactions = \
            CombinatorialConformation.update_reactions(self)
        promoter_reactions = []
        old_name = self.name
        for s in self.conformation_species:
            if isinstance(s, PolymerConformation):
                active_state = False
                if  (s in self.promoter_states and self.promoter_states_on) \
                        or (s not in self.promoter_states and
                            not self.promoter_states_on):
                    active_state = True
                ...
                if any([s.get_complex(c) is not None
                        for c in self.inactivating_complexes]):
                    innactive_complex = True
                    if (active_state and len(self.promoter_states) > 0) \
                       or active_complex:
                        warnings.warn(
                            "inactive_complex conflicts with active_complex "
                            "or active_state in "
