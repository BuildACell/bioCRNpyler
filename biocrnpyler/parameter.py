...
class Parameter(object):
    ...
    def get_parameters(config_file, custom, default={}, **keywords):
        ...
        if config_file != None:
            config_parameters = Parameter.load_config(config_file)
        ...
        if custom != None:
            for key, value in custom.items():
                parameters[key] = Parameter._to_parameter(key, value)
    ...
    def update_missing(existing_dict, default_dict):
        ...
        for key, value in default_dict.items():
            if key not in existing_dict.keys():
                existing_dict[key] = Parameter._to_parameter(key, value)
    ...
    def create_parameter_dictionary(parameters, parameter_file):
        ...
        for fname in file_list:
            ...
            try:
                pID_ind = Parameter.get_flexible_string_list_index(L0, ["part_id", "part"])
                if pID_ind == None:
                ...
                raise ValueError("'part_id', 'part', or a similar string appears "
                                 ...
                                 "keyword list = {L0}.")
            try:
                mech_ind = Parameter.get_flexible_string_list_index(L0, ["mechanism",
                                                                         "mechanism_id"])
                if mech_ind == None:
            ...
            try:
                param_ind = Parameter.get_flexible_string_list_index(L0,
                ...
                raise ValueError("'param', 'parameter', 'parameter_name', "
                                 ...
                                 "keyword list = {L0}")
            try:
                val_ind = Parameter.get_flexible_string_list_index(L0,
