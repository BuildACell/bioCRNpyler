...
class GlobalMechanism(Mechanism):
    def __init__(self, name, mechanism_type = "", filter_dict = {},
                 default_on = False):
        self.filter_dict = filter_dict
        self.default_on = default_on
        Mechanism.__init__(self, name=name, mechanism_type=mechanism_type)

    #applies the filter dictionary to determine if a global mechanism acts on a
    # species
    def apply_filter(self, s):
        fd = self.filter_dict
        use_mechanism = None

        for a in s.attributes+[s.material_type, repr(s), s.name]:
            if a in fd:
                if use_mechanism == None:
                    use_mechanism = fd[a]
                elif use_mechanism != fd[a]:
                    raise AttributeError(f"species {repr(s)} has multiple "
                                         "attributes(or material type) which "
                                         "conflict with global mechanism "
                                         "filter {repr(self).")
...
# Global Mechanism to Constutively Create Certain Species at the rate of
# dilution.
