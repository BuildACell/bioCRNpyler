...
class CombinatorialConformation(Component):
    ...
    def __init__(self, final_states, initial_states = None, intermediate_states = None, excluded_states = None, state_part_ids = None, name = None, **keywords):
        ...
        mechanisms = [One_Step_Reversible_Conformation_Change()]
        Component.__init__(self, name = name, mechanisms = mechanisms, **keywords)
    ...
    def compute_species_changes(self, s0, sf):
        ...
        #Compute mapping between complexes and locations in the polymer
        ind2c0 = {}
        ind2cf = {}
        cf2ind = {}
        c02ind = {}
        for c0 in s0.complexes:
            p_inds = s0.get_polymer_positions(c0, 0) #polymer index is always 0
            c02ind[c0] = p_inds
            for i in p_inds:
                if i is not None:
                    ind2c0[i] = c0
        ...
        for cf in sf.complexes:
            p_inds = sf.get_polymer_positions(cf, 0) #polymer index is always 0
            cf2ind[cf] = p_inds
            for i in p_inds:
                if i is not None:
                    ind2cf[i] = cf
        ...
        if len([i for i in ind2c0 if i not in ind2cf]) > 0:
            species_changes = False
        else:
            species_changes = {}
            merged_complexes = {cf:[] for cf in sf.complexes}
            ...
            for cf in sf.complexes:
                ...
                cf_species_and_inds = [(cf.species[i], cf2ind[cf][i])  for i in range(len(cf.species))]
                ...
                for ind in cf2ind[cf]:
                    c0 = ind2c0.get(ind) #check if there is a complex in s0 at this location

                    #there is no complex c0 at ind in s0, so do nothing
                    if c0 is None:
                        pass
                    ...
                    elif c0 not in merged_complexes[cf]:
                        if cf in merged_complexes:
                            merged_complexes[cf].append(c0)
                        else:
                            merged_complexes[cf] = [c0]
                        sub_complex_species_and_inds += [(c0.species[i], c02ind[c0][i])  for i in range(len(c0.species))]
                        sub_complex_species_and_inds_str += [str((c0.species[i], c02ind[c0][i]))  for i in range(len(c0.species))]

                #cf cannot be created additively from the complexes in s0
                if len([i for i in sub_complex_species_and_inds_str if i not in cf_species_and_inds_str]) > 0:
                    species_changes[cf] = False
                #keep track of all the external species added to produce the new complex
                elif len([i for i in cf_species_and_inds_str if i not in sub_complex_species_and_inds_str]) > 0:
                    species_changes[cf] = [cf_species_and_inds[i][0] for i in range(len(cf_species_and_inds_str)) if cf_species_and_inds_str[i] not in sub_complex_species_and_inds_str and cf_species_and_inds[i][1] is None]
                
        #If any complex cannot be created, return False
        if species_changes == False or any([species_changes[cf] is False for cf in species_changes]) or (len(species_changes) == 0 and len(merged_complexes) == 0):
            return False
        else:
    ...
    def get_combinations_between(self, s0, sf):
        ...
        print("\ncomputing combinations between", s0, "and", sf)

        if X is False:
            print("No species changes")
            return []
        else:
            species_changes, merged_complexes = X
            print("species_changes=", species_changes, "merged_complexes=", merged_complexes)
            changes_list = []
            for cf in sf.complexes:
                inds = sf.get_polymer_positions(cf, 0)

                if (cf in merged_complexes and len(merged_complexes[cf]) == 1 and merged_complexes[cf][0].monomer_eq(cf)) or cf not in merged_complexes:
                    complexes_to_merge = None
                else:
                    complexes_to_merge = merged_complexes[cf]

                if cf not in species_changes or len(species_changes[cf]) == 0:
                    species_to_add = None
                else:
                    species_to_add = species_changes[cf]

                if species_to_add is not None or complexes_to_merge is not None:
                    print("adding to changes list:", (inds, species_to_add, complexes_to_merge))
            ...
            for perm in perms:
                ...
                for cf, inds, species_to_add, complexes_to_merge in perm:
                    ...
                    print("old polymer conformation", old_state)
                    print("old complexes", old_state.complexes)
                    
                    print("species list", species_list)
                    new_complex = Complex(species_list)

                    print("new complex", new_complex)
                    ...
                    if merged_complexes not in [[], None]:
                        new_state = new_complex.parent.remove_complexes(merged_complexes)
                    else:
                        new_state = new_complex.parent
                    print("new polymer conformation", new_state)
    ...
    def update_species(self):
        ...
        if self.intermediate_states is not None:
            print("There are intermediate states")
...
class CombinatorialConformationPromoter(CombinatorialConformation, Promoter):
    ...
    This class merges Promoter and CombinatorialConformation.
        ...
        :param expressing_states: one ore more PolymerConformations which can be transcribed
    ...
    """
    def __init__(expressing_states, promoter_location, rbs = None, promoter = None, **keywords):
        CombinatorialConformation.__init__(self, **keywords)
        Promoter.__init__(self, **keywords)

        self.expressing_states = expressing_states
    ...
    #expressing_states are PolymerConformations which are allow to transcribe and translate RNA/proteins
    @property
    def expressing_states(self):
        return self._expressing_states

    @expressing_states.setter
    def expressing_states(self, expressing_states):
        if expressing_states is None:
            self._expressing_states = []
        else:
            #All excluded states must be PolymerConformations
            expressing_states = list(self.set_species(expressing_states))
            self._assert_conformation(expressing_states, "expressing_states")
            self._expressing_states = expressing_states


    def update_species(self):
        promoter_species = []
        for s in self.expressing_states:
            self.dna_to_bind = s.polymers[0][self.promoter_location]
            promoter_species += Promoter.update_species(self)

        conformation_species = CombinatorialConformation.update_species(self)
        return promoter_species+conformation_species

    def update_reactions(self):
        promoter_reactions = []
        for s in self.expressing_states:
            self.dna_to_bind = s.polymers[0][self.promoter_location]
            promoter_reactions += Promoter.update_species(self)
