...
class Membrane_Signaling_Pathway_MM(Mechanism):
    ...
    def update_species(self, membrane_sensor_protein, response_protein, assigned_substrate, 
        ...
        if complex_dict is None:
            ...
            complex_dict={}
            complex_dict['Activated_MP']=Complex([signal_substrate, 
                                                    membrane_sensor_protein])
            complex_dict['ATP:Activated_MP']=Complex([nATP*[energy], 
                                                        complex_dict['Activated_MP']])
            complex_dict['ADP:Activated_MP:Sub']=Complex([complex_dict['Activated_MP'],
                                                            nATP*[waste], 
                                                            assigned_substrate])
            complex_dict['Activated_MP:Sub']=Complex([complex_dict['Activated_MP'], 
                                                        assigned_substrate])
            complex_dict['Activated_MP:Sub:RP']=Complex([complex_dict['Activated_MP:Sub'], 
                                                            response_protein])
            complex_dict['Activated_MP:RP:Sub']=Complex([complex_dict['Activated_MP'], 
                                                            response_protein, assigned_substrate])
        #Make dictionary into a list 
        complex_array = [value for value in complex_dict.values()]
            
        return [membrane_sensor_protein, response_protein, 
                assigned_substrate, signal_substrate, energy, waste, complex_array]
