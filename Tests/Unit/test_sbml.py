...
class TestSBML(TestCase):
    ...
    def test_add_all_species(self):
        ...
        self.assertEqual(
            str(
                getSpeciesByName(
                    model, model.getSpecies(0).getName()
                ).getName()
            ),
            'S1',
        )
    ...
    def test_add_all_compartments(self):
        ...
        S4 = Species(
            'S4', compartment=S4_compartment, material_type='protein'
        )
        ...
        self.assertEqual(
            len(model.getListOfCompartments()), len(list_of_compartments)
        )
        for compartment, size, dim in zip(
            model.getListOfCompartments(), list_of_sizes, list_of_dimensions
        ):
    ...
    def test_add_reaction(self):
        ...
        propensities = [
            MassAction(k_forward=1.0),
            MassAction(k_forward=1, k_reverse=0.1),
            MassAction(k_forward=k1),
            MassAction(k_forward=k1, k_reverse=k2),
            HillPositive(k=1, n=2.0, K=3.0, s1=S1),
            HillPositive(k=k1, n=2.0, K=k2, s1=S1),
            HillNegative(k=1, n=2.0, K=3.0, s1=S1),
            HillNegative(k=k1, n=k2, K=3.0, s1=S1),
            ProportionalHillPositive(k=1, n=2, K=3.0, s1=S1, d=S2),
            ProportionalHillPositive(k=k1, n=2, K=k2, s1=S1, d=S2),
            ProportionalHillNegative(k=1, n=2, K=3.0, s1=S1, d=S2),
        ...
        for prop in propensities:  # Cycle through different propensity types
            for (
                ...
                for stochastic in [True, False]:  # Toggle Stochastic
                    rxn_num = 0
                    model_id = (
                        f'{prop.name}_model_with_stochastic_{stochastic}'
                    )
                    ...
                    try:
                        ...
                        if len(crn_reactants) > 0:
                            assert all(
                                crn_reactants.count(s.getSpecies())
                                == int(s.getStoichiometry())
                                for s in sbml_reaction.getListOfReactants()
                            )

                        if len(crn_products) > 0:
                            assert all(
                                crn_products.count(s.getSpecies())
                                == int(s.getStoichiometry())
        ...
        error_propensities = [
            MassAction(k_forward=k1_err),
            HillPositive(k=k1_err, n=2.0, K=3.0, s1=S1),
            GeneralPropensity(
                'k1*2 - k2/S1^2*',
                propensity_species=[S1],
                propensity_parameters=[k1, k2],
        ...
        for prop in error_propensities:
            with pytest.raises(
                ValueError,
                match='Could not write the rate law for reaction to SBML. Check the propensity functions of reactions.',
            ):
...
def test_generate_sbml_model():
    ...
    assert len(model2.getListOfSpecies()) == len(
        crn.species
    )  # the same model is made if you dont check validity
...
def test_sbml_basics():
    ...
    sbml_species = add_species(
        ...
        compartment,
        str(S1.name),
        trans.getValidIdForName(str(S1)),
    ...
    sbml_species2 = add_species(
        ...
        compartment,
        str(S2.name),
        trans.getValidIdForName(str(S2)),
        initial_concentration=10,
    )
    ...
    check(
        sbml_species2.getInitialConcentration(),
        'get concentration for SBML species',
    )
    ...
    check(
        modifier.getSpecies(), 'get the species id for the modifier reference'
    )
    ...
    with pytest.raises(
        ValueError, match='Units for a parameter must be passed as strings.'
    ):
    ...
    check(
        rateLaw.setFormula('k_local*10 - k_global/2'),
