...
def test_promoter_DNAconstruct():
    ...
    mechs = {
        'transcription': Transcription_MM(
            Species('RNAP', material_type='protein')
        )
    }
    ...
    x = DNA_construct(
        [[P, 'reverse']], mechanisms=mechs, parameters=parameters
    )
    assert not (P in x.parts_list)  # make sure P is copied
    assert P in x  # testing "contains" function of Construct
    y = x.enumerate_components()
...
def test_promoter_terminator_DNAconstruct():
    ...
    x = DNA_construct(
        [[P, 'reverse'], T], mechanisms=mechs, parameters=parameters
    )
    y = x.enumerate_components()
    ...
    x = DNA_construct(
        [T, [P, 'reverse']], mechanisms=mechs, parameters=parameters
    )
    y = x.enumerate_components()
...
def test_get_part_DNAconstruct():
    ...
    x = DNA_construct(
        [P, U, C, T, T2], mechanisms=mechs, parameters=parameters
    )
    with pytest.raises(
        ValueError,
        match=r'get_component requires a single keyword. Recieved component=None, name=None, index=None.',
    ):
...
def test_circular_DNAconstruct():
    ...
    # circular construct
    x = DNA_construct(
        [P, T], mechanisms=mechs, parameters=parameters, circular=True
    )
    y = x.enumerate_components()
    assert y[0] == x[0]  # correct promoter is returned
    assert (
    ...
    z = DNA_construct(
        [T, P], mechanisms=mechs, parameters=parameters, circular=True
    )
...
def test_combinatorial_enumeration_DNAconstruct():
    ...
    # circular construct
    x = DNA_construct(
        [P, P, T], mechanisms=mechs, parameters=parameters, circular=True
    )
    ...
    p1_bound = Complex(
        [x.get_species()[0], Species('RNAP', material_type='protein')]
    ).parent
    p2_bound = Complex(
        [x.get_species()[1], Species('RNAP', material_type='protein')]
    ).parent
    both_bound = Complex(
        [p1_bound[1], Species('RNAP', material_type='protein')]
    ).parent
    assert (
        x.get_species() in dna_species[0]
    )  # unbound polymer found in combinatorial enumeration
    assert (
        p2_bound in dna_species[0]
    )  # proper bound polymer found in combinatorial enumeration
    assert (
        x.get_species() in dna_species[1]
    )  # unbound polymer found in combinatorial enumeration
    assert (
        p1_bound in dna_species[1]
    )  # proper bound polymer found in combinatorial enumeration
    assert (
