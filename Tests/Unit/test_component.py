...
class TestComponent(TestCase):
    def setUp(self) -> None:
        ...
        self.component = Component(
            ...
            mixture=None,
            attributes=[],
            initial_concentration=self.default_concentration,
        )

    def test_initial_concentration(self):
        # test that the default initial concentration is zero
        self.assertEqual(
            self.component.initial_concentration, self.default_concentration
        )
        # test there is one entry in the parameter database (the initial concentration)
        self.assertTrue(
            len(self.component.parameter_database.parameters) == 1
        )
        param = self.component.parameter_database.find_parameter(
            mechanism='initial concentration',
            part_id=None,
            param_name=self.comp_name,
        )
    ...
    def test_update_parameters(self):
        ...
        self.assertTrue(
            isinstance(component.parameter_database, ParameterDatabase)
        ...
        self.assertTrue(
            len(component.parameter_database.parameters) == len(parameters)
        )
        ...
        component.update_parameters(
            parameters=one_param, overwrite_parameters=True
        )
        self.assertEqual(
            component.parameter_database[(None, None, 'kb')].value, new_val
        )
    ...
    def test_add_mechanism(self):
        ...
        # add a non-mechanism
        with self.assertRaisesRegex(
            TypeError, 'mechanism must be a Mechanism.'
        ):
        ...
        with self.assertRaisesRegex(
            TypeError, 'mechanism keys must be strings.'
        ):
            self.component.add_mechanism(tx, mech_type=1234)

        # add a mechanism already in the Component
        with self.assertRaisesRegex(
            ValueError, f'mech_type {tx.mechanism_type} already in component'
        ):
            self.component.add_mechanism(tx)

        # add mechanism with optional_mechanism - does not overwrite!
        self.component.add_mechanism(
            tl, mech_type=tx.mechanism_type, optional_mechanism=True
        )
        self.assertTrue(tx.mechanism_type in self.component.mechanisms)
        self.assertTrue(
        ...
        self.component.add_mechanism(
            tl, mech_type=tx.mechanism_type, overwrite=True
        )
        self.assertTrue(tx.mechanism_type in self.component.mechanisms)
        self.assertTrue(
            type(self.component.mechanisms[tx.mechanism_type])
    ...
    def test_update_mechanisms(self):
        ...
        self.assertEqual(
            type(self.component.mechanisms[tx.mechanism_type]),
            type(test_mech[tx.mechanism_type]),
        )
        self.assertFalse(
            self.component.mechanisms[tx.mechanism_type]
            == test_mech[tx.mechanism_type]
        )
    ...
    def test_get_parameter(self):
        ...
        self.assertEqual(
            self.component.get_parameter(param_name='kb').value, kb
        )
        self.assertEqual(
            self.component.get_parameter(
                mechanism=tx, param_name='ktx'
            ).value,
            ktx,
        )
        self.assertEqual(
            self.component.get_parameter(
                part_id=p_id, param_name='ktl'
            ).value,
            ktl,
        )
        self.assertEqual(
            self.component.get_parameter(
                mechanism=tx, part_id=p_id, param_name='ku'
            ).value,
            ku,
        )
        ...
        self.assertEqual(
            self.component.get_parameter(
                param_name='kb', return_numerical=True
            ),
            kb,
        )
        ...
        self.assertEqual(
            self.component.get_parameter(param_name='kb').value,
            one_param['kb'],
        )
    ...
    def test_get_mechanism(self):
        ...
        with self.assertRaisesRegex(
            KeyError, 'Unable to find mechanism of type'
        ):
            C_copy.get_mechanism('DNE')

        # test get_mechanism with no_key_error = True
        self.assertTrue(
            C_copy.get_mechanism('DNE', optional_mechanism=True) is None
        )
    ...
    def test_set_species(self):
        ...
        self.assertEqual(
            str(s1), str(s2)
        )  # A Species derived from a string will have no material_type
