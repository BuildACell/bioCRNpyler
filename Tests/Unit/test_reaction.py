...
class TestReaction(TestCase):
    def test_reaction_initialization(self):
        ...
        rxn = Reaction.from_massaction(
            inputs=[], outputs=[], k_forward=0.1, k_reverse=1
        )
        ...
        Reaction(
            inputs=[chem_com_sp1],
            outputs=[chem_com_sp2],
            propensity_type=MassAction(k_forward=1),
        )
...
def test_species_merging():
    sp1 = Species(name='test_species_a')
    chem_complexes = [
        WeightedSpecies(species=sp1, stoichiometry=2),
        WeightedSpecies(species=sp1, stoichiometry=1),
    ]
    rxn = Reaction(
        inputs=chem_complexes,
        outputs=[],
        propensity_type=MassAction(k_forward=1),
    )
...
def test_reaction_equality():
    ...
    rxn3 = Reaction(
        inputs=[sp2, sp1],
        outputs=[sp2, sp2],
        propensity_type=MassAction(k_forward=10),
    )
    rxn4 = Reaction(
        inputs=[sp2, sp1],
        outputs=[sp2],
...
def test_reaction_list_flattening():
    ...
    rxn2 = Reaction(
        inputs=[sp1, sp1, sp2], outputs=[sp1, sp2, sp2], propensity_type=mak
    )
    assert rxn1 == rxn2
