...
class TestOrderedMonomer(TestCase):
    def test_ordered_monomer_initialization(self):
        ...
        self.assertEqual(
            OrderedMonomer(direction='reverse'), x.set_dir('reverse')
        )

        # Bad parent
        with self.assertRaisesRegex(
            ValueError, 'parent must be an MonomerCollection'
        ):
...
class TestOrderedPolymer(TestCase):
    def test_ordered_polymer_intialization(self):
        x = OrderedMonomer(direction='forward')
        y = OrderedPolymer(
            [x, [OrderedMonomer(), 'reverse'], OrderedMonomer()]
        )
...
class TestOrderedPolymerSpecies(TestCase):
    ...
    def test_ordered_polymer_species_initialization(self):
        a = Species('A')
        x = OrderedPolymerSpecies(
            [
                Species('A'),
                [Species('B'), 'forward'],
                Species('C').set_dir('reverse'),
            ],
            attributes=['ooga'],
        )
        # repr
        revtest = OrderedPolymerSpecies(
            [Species('A', direction='forward'), Species('A')]
        )
        r1 = str(revtest)
        revtest.reverse()
        r2 = str(revtest)
    ...
    def test_ordered_polymer_species_manipulations(self):
        ...
        reversd = OrderedPolymerSpecies(
            [
                Species('C').set_dir('forward'),
                [Species('B'), 'reverse'],
                Species('A'),
            ],
            attributes=['ooga'],
        )
        unreplaced = OrderedPolymerSpecies(
            [
                Species('z'),
                [Species('B'), 'forward'],
                Species('C').set_dir('reverse'),
        ...
        uninserted = OrderedPolymerSpecies(
            [Species('A'), Species('C').set_dir('reverse')],
            attributes=['ooga'],
        )
        unappended = OrderedPolymerSpecies(
            [
                Species('A'),
                [Species('B'), 'forward'],
            ],
