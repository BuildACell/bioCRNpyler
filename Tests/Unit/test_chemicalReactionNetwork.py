...
class TestChemicalReactionNetwork(TestCase):
    def setUp(self) -> None:
        ...
        self.rx1 = Reaction.from_massaction(
            inputs=[self.s1], outputs=[self.s2], k_forward=0.1
        )
    ...
    def test_check_crn_validity(self):
        checked_reactions, checked_species = (
        ...
        rxn2 = Reaction.from_massaction(
            inputs=[self.s1], outputs=[self.s3], k_forward=0.1
        )
        # test warning raised if a species (in the reaction outputs) is detected which is not part of the species list
        with self.assertWarnsRegex(
            Warning, 'are not part of any reactions in the CRN'
        ):
            ChemicalReactionNetwork.check_crn_validity(
                reactions=[rxn2],
        ...
        rxn3 = Reaction.from_massaction(
            inputs=[self.s4], outputs=[self.s2], k_forward=0.1
        )
        ...
        rxn4 = Reaction.from_massaction(
            inputs=[self.s4, self.s3], outputs=[self.s2], k_forward=0.1
        )
        with self.assertWarnsRegex(
            Warning,
            'are not listed in the Species list, but part of the reactions',
        ):
            ChemicalReactionNetwork.check_crn_validity(
                reactions=[rxn4],
        ...
        CRN = ChemicalReactionNetwork(
            species=[self.s1, self.s2], reactions=rxn_list
        )
        self.assertTrue(CRN.reactions.count(self.rx1) == 2)

        with self.assertWarnsRegex(
            Warning, 'may be duplicated in CRN definitions'
        ):
            ChemicalReactionNetwork.check_crn_validity(
                reactions=rxn_list,
                species=self.species_list,
    ...
    def test_replace_Species_in_ComplexSpecies(self):
        ...
        self.assertTrue(
            ...
            == Complex([self.s1, self.s_new])
        )
        self.assertTrue(c2 == Complex([self.s1, c1]))
        self.assertTrue(
            c2.replace_species(self.s_new, self.s_old)
            == Complex([self.s1, Complex([self.s1, self.s_old])])
        )

    def test_replace_Species_in_OrderedComplexSpecies(self):
        oc1 = Complex([self.s1, self.s_old], ordered=True)
        self.assertTrue(
            oc1.replace_species(self.s_old, self.s_new)
            == Complex([self.s1, self.s_new], ordered=True)
        )
    ...
    def test_replace_species_in_Reaction(self):
        ...
        r1 = Reaction.from_massaction(
            [self.s1, self.s_old], [c1], k_forward=1
        )
        self.assertTrue(
            r1.replace_species(self.s_old, self.s_new)
            == Reaction.from_massaction(
                [self.s1, self.s_new], [c2], k_forward=1
            )
        )
    ...
    def test_replace_species_with_a_non_massaction_reaction(self):
        ...
        prop_hill_old = ProportionalHillPositive(
            k=1.0, s1=self.s1, K=10, d=self.s_old, n=2
        )
        r1 = Reaction(
            [self.s1, self.s_old], [c1], propensity_type=prop_hill_old
        )
        prop_hill_new = ProportionalHillPositive(
            k=1.0, s1=self.s1, K=10, d=self.s_new, n=2
        )
        r1_new = Reaction(
            [self.s1, self.s_new],
            [c1.replace_species(self.s_old, self.s_new)],
    ...
    def test_replace_in_a_chemical_reaction_network(self):
        ...
        r1_new = Reaction.from_massaction(
            [self.s1, self.s_new], [c1_new], k_forward=1
        )
    ...
    def test_write_sbml_file(self):
        ...
        rx1 = Reaction.from_massaction(
            inputs=[s1], outputs=[s2], k_forward=0.1
        )
