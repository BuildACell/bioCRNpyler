...
def test_complex_with_multiple_polymers():
    ...
    # Ordered Case
    oc = Complex(
        [Species('S'), pc0.polymers[0][0], pc0.polymers[0][2]], ordered=True
    )
    # Correct parent
    assert oc.parent == PolymerConformation(
        [
            OrderedComplexSpecies(
                [Species('S'), p[0], p[2]], called_from_complex=True
            )
        ]
    )
    # correct complex returned
    ...
    assert c2.parent == PolymerConformation(
        [
            ComplexSpecies(
                [Species('S'), p[0], p2[1]], called_from_complex=True
            )
        ]
    )
    assert str(c2) == str(
...
def test_complex_with_polymer_replacement():
    ...
    pc0 = PolymerConformation(
        polymer=p
    )  # Put the polymer inside a conformation
    # Bind two monomers together
    pc = Complex(
        [pc0.polymers[0][0], pc0.polymers[0][1]]
    ).parent  # get a PolymerConformation
    ...
    assert (
        c.parent.parent
        != Complex([pc2.polymers[0][0], pc2.polymers[0][1]]).parent
    )
...
def test_complex_with_a_complex_in_a_conformation():
    ...
    pc0 = PolymerConformation(polymer=p)
    pc = Complex(
        [pc0.polymers[0][0], pc0.polymers[0][1]], ordered=True
    ).parent  # get a PolymerConformation
    c = pc.complexes[0]  # get a complex from the PolymerConformation

    c2 = Complex(
        [c, Species('S')], ordered=True
    )  # Create a Complex with a Complex
    pc2 = c2.parent
    assert str(c2) == str(
        OrderedComplexSpecies([p[0], p[1], Species('S')])
    )  # merging done correctly
    assert (
        pc2
        == Complex(
            [pc0.polymers[0][0], pc0.polymers[0][1], Species('S')],
            ordered=True,
        ).parent
    )  # check the parent PolymerConformation
    assert len(pc2.complexes) == 1

    # Create a PolymerConformation with two complexes
    c3 = Complex(
        [pc2.polymers[0][0], pc2.polymers[0][2], Species('S2')], ordered=True
    )
    pc3 = c3.parent
    assert len(pc3.complexes) == 2
    assert str(c3) == str(
        OrderedComplexSpecies(
            [p[0], p[2], Species('S2')], called_from_complex=True
        )
    )

    # merge the two complexes in pc3
    c4 = Complex(
        [pc3.complexes[0], pc3.complexes[1], Species('S3')], ordered=True
    )
