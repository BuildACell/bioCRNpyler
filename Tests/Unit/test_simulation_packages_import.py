...
def test_bioscrape_import_simulate():
    ...
    try:
        ...
        with pytest.warns() as record:
            sim_result = CRN.simulate_with_bioscrape(
        ...
        if sim_result is None:
            ...
            assert (
                'simulate_with_bioscrape is depricated and will cease working in a future release.'
...
def test_bioscrape_import_simulate_via_sbml():
    ...
    try:
        ...
        with warnings.catch_warnings(record=True) as record:
            sim_result, bioscrape_model = (
                CRN.simulate_with_bioscrape_via_sbml(
                    timepoints=np.linspace(0, 10, 100),
                    initial_condition_dict={str(X): 1},
                    return_model=True,
                )
...
def test_libroadrunner_import():
    ...
    try:
        ...
        with pytest.warns(
            UserWarning,
            match='libroadrunner was not found, please install libroadrunner',
        ):
        ...
        sim_results = CRN.simulate_with_roadrunner(
            timepoints=list(range(0, 10))
        )
