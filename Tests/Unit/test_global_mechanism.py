...
class TestGlobalMechanism(TestCase):
    def setUp(self) -> None:
        ...
        self.mechanism = GlobalMechanism(
    ...
    def test_default_filtering(self):
        ...
        mech_default_on = GlobalMechanism(
            name=self.mech_name, mechanism_type='dummy', default_on=True
        )
        self.assertTrue(
            False
            not in [mech_default_on.apply_filter(s) for s in [s1, s2, c1, c2]]
        )

        # Always OFF for all species
        mech_default_off = GlobalMechanism(
            name=self.mech_name, mechanism_type='dummy', default_on=False
        )
        self.assertTrue(
            True
            not in [
    ...
    def test_filter_dictionary(self):
        ...
        mech_default_on_fds1 = GlobalMechanism(
            name=self.mech_name,
        ...
        self.assertTrue(mech_default_on_fds1.apply_filter(s2))
        self.assertTrue(
            mech_default_on_fds1.apply_filter(c1)
        )  # attributes are NOT inherited through ComplexSpecies
        self.assertTrue(mech_default_on_fds1.apply_filter(c2))
    ...
    def test_recursive_filtering(self):
        ...
        self.assertTrue(
            mech_default_on_fds1.apply_filter(c1)
        )  # attributes are not inherited through ComplexSpecies, but contained inside
        self.assertTrue(mech_default_on_fds1.apply_filter(c2))

    def test_rna_degradation_mm(self):
        ...
        rdmm = Degradation_mRNA_MM(rnaase)
        ...
        # reactions
        self.assertTrue(
            all([str(rxns[i]) == str(rxns_mm[i]) for i in range(len(rxns))])
        )

        # Overwrite default_on, recursive_species_filtering, and filter_dict
        rdmm2 = Degradation_mRNA_MM(
            rnaase,
            default_on=True,
            recursive_species_filtering=False,
        ...
        self.assertTrue(
            'test' in rdmm2.filter_dict and rdmm2.filter_dict['test'] is True
        )
        self.assertTrue(len(rdmm2.filter_dict) == 1)

    def test_deg_tagged_degradation(self):
        M = Mixture(parameters={'kdeg': 1, 'kb': 1, 'ku': 1})
        ...
        dtd = Deg_Tagged_Degradation(protease)
        # test default global mechanism parameters
        ...
        self.assertTrue(
            'test' in dtd2.filter_dict and dtd2.filter_dict['test'] is True
        )
