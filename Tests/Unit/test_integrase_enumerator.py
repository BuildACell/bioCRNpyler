...
def test_polymer_transformation():
    ...
    assert transformation.create_polymer(
        ...
        [prom, prom, cds2], circular=True
    )  # transformations are performed correctly
    assert transformation.create_polymer(
        [construct2, construct]
    ) == DNA_construct([cds2, prom, prom], circular=True)
    assert '(input1-0)(p1)(input2-0)' in str(
        transformation
    )  # contains vital information in the repr
...
def test_integrase_rule():
    ...
    assert bxb1_rule.integrase_species == Species(
        'Bxb1', material_type='protein'
    )

    bxb1_enumerator = Integrase_Enumerator(
        'Bxb1', int_mechanisms={'Bxb1': bxb1_rule}
    )

    flp_rule = IntegraseRule(name='FLP', reactions={('FLP', 'FLP'): 'FLP'})  # noqa: F841
    bad_rule = IntegraseRule(
        name='Bxb1', reactions={('attB', 'attP'): 'attL'}
    )
    with pytest.raises(AssertionError):
        bxb1_rule.generate_products(
            delete[0], delete[2]
        )  # must have the right integrase
    delete[0].update_integrase('Bxb1')
    delete[2].update_integrase('Bxb1')
    with pytest.raises(KeyError):
        bxb1_rule.generate_products(
            delete[0], delete[0]
        )  # attP+attP is not possible
    with pytest.raises(KeyError):
        bad_rule.generate_products(
            delete[0], delete[2]
        )  # attP->attB reaction is not possible with a bad rule
    ...
    new_constructs = bxb1_rule.integrate(
        flip[0], flip[2], also_inter=False
    )  # sites forwards
    assert len(new_constructs) == 1  # one product is made
    assert [part.name for part in new_constructs[0]] == [
        'attL',
        'GFP',
        'attR',
    ]
    ...
    new_constructs = bxb1_rule.integrate(
        flip[2], flip[0], also_inter=False
    )  # sites reverse
    assert len(new_constructs) == 1  # one product is made
    assert [part.name for part in new_constructs[0]] == [
        'attL',
    ...
    new_constructs = bxb1_rule.integrate(
        flipr[2], flipr[0], also_inter=False
    )  # sites forwards
    assert len(new_constructs) == 1  # one product is made
    assert [part.name for part in new_constructs[0]] == [
        'attR',
    ...
    new_constructs = bxb1_rule.integrate(
        flip[0], flip[2], also_inter=True
    )  # allow intermolecular reactions
    assert len(new_constructs) == 2  # now, two products are made
    assert (
        sum([a.circular for a in new_constructs]) == 2
    )  # all of them is circular
    ...
    # deletion
    new_constructs = bxb1_rule.integrate(
        delete[0], delete[2], also_inter=False
    )  # sites forwards
    assert len(new_constructs) == 2  # two product is made
    assert (
        sum([a.circular for a in new_constructs]) == 1
    )  # one of them is circular
    for prod in new_constructs:
        if prod.circular:
    ...
    new_constructs = bxb1_rule.integrate(
        delete[2], delete[0], also_inter=True
    )  # sites reverse
    ...
    new_constructs = bxb1_rule.integrate(
        plasp[1], plasb[1]
    )  # two plasmids -> one plasmid p then b
    assert len(new_constructs) == 1  # one product is made
    ...
    assert (
        len(plasb[1].linked_sites) == 0
    )  # linked sites are nonexistant if it is reset
    assert (
        len(plasp[1].linked_sites) == 0
    )  # linked sites are nonexistant if it is reset

    # include result construct
    result_construct = DNA_construct([cds, aL, cds, aR], circular=True)
    new_constructs = bxb1_rule.integrate(
        plasb[1], plasp[1], existing_dna_constructs=[result_construct]
    )  # two plasmids -> one plasmid b then p
    assert (
        len(new_constructs) == 0
    )  # no new products are made, only the existing one
    assert (plasp[1], True) in plasb[
        1
    ].linked_sites  # proper reaction found inside integrase site
    assert (plasb[1], True) in plasp[
        1
    ].linked_sites  # proper reaction found inside integrase site
    ...
    new_constructs = bxb1_rule.integrate(
        plasb[1], plasp[1], existing_dna_constructs=[permuted_construct]
    )  # two plasmids -> one plasmid b then p
    assert (
        len(new_constructs) == 0
    )  # no new products are made, only the existing one
    ...
    assert (
        sum([a.circular for a in new_constructs]) == 0
    )  # all products are linear
    for prod in new_constructs:
        assert [part.name for part in prod] in [
            ['RFP', 'attL'],
            ['RFP', 'attR'],
        ]
        assert [part.direction for part in prod] == ['forward', 'forward']
...
def test_integrase_enumerator():
    ...
    assert (
        flip[0] in int_dict['Bxb1']
    )  # both sites are found in the dictionary
    assert (
        flip[2] in int_dict['Bxb1']
    )  # both sites are found in the dictionary

    x = bxb1_rule.integrate(flip[0], flip[2])
    assert (flip[2], False) in flip[
        0
    ].linked_sites  # sites are properly integrated
    ...
    assert [perm_func(a) for a in range(len(plasp))] == [
        (1, 'f'),
        (0, 'f'),
    ]  # find permuted mapping
    assert found == permuted_plasp  # found the same construct we put in
    assert (
        bxb1_enumerator.find_dna_construct(plasp, [plasb]) is None
    )  # we don't find anything

    reversed_plasp = DNA_construct(
        [(ap, 'reverse'), (cds, 'reverse')], circular=True
    )
    reversed_permuted_plasp = DNA_construct(
        [(cds, 'reverse'), (ap, 'reverse')], circular=True
    )
    found, perm_func = bxb1_enumerator.find_dna_construct(
    ...
    assert [perm_func(a) for a in range(len(genp))] == [
        (1, 'r'),
        (0, 'r'),
    ]  # find permuted mapping
    ...
    y = bxb1_enumerator.enumerate_components(
        [plasp, plasb], previously_enumerated=[plaspb]
    )
    assert (
        y == []
    )  # nothing new is made since the proper construct has been "previously enumerated"

    plaspb = DNA_construct(
        [ar, cds, al, cds, ar, cds, ar, cds], circular=True
    )
...
def test_compilation():
    ...
    bxb1_mechanism = IntegraseRule(
        'Bxb1', reactions={('attB', 'attP'): 'attL', ('attP', 'attB'): 'attR'}
    )
    ...
    myCRN0, comps0 = myMixture.compile_crn(
        recursion_depth=0,
    ...
    myCRN1, comps1 = myMixture.compile_crn(
        recursion_depth=1,
        return_enumerated_components=True,
        initial_concentrations_at_end=True,
        copy_objects=False,
