...
def test_polymer_conformation_no_complex_instantiation():
    ...
    pc2 = PolymerConformation(
        polymer=[Species('m1'), Species('m2'), Species('m3')]
    )
    assert pc2 == pc
...
def test_multiple_polymer_multiple_complex_instantiation():
    ...
    c3 = ComplexSpecies(
        [Species('S1'), Species('S2')], called_from_complex=True
    )
    ...
    assert (
        str(pc1b) == f'conformation__{p1}_{p2}_p0l0p1l1_{c1}_p0l0p1l2_{c2}_'
    )
    assert pc1 != pc1b
...
def test_from_polymer_conformation():
    ...
    c1b = ComplexSpecies(
        [pc1.polymers[0][0], pc1.polymers[0][2]], called_from_complex=True
    )
    ...
    assert (
        pc1b
        == PolymerConformation([c1, c1bb])
        == PolymerConformation([c1bb, c1])
    )
    ...
    c4 = ComplexSpecies(
        [pc3.polymers[0][0], pc2.polymers[0][1]]
    ...
    assert (
        pc4
        == PolymerConformation([c2, c3, c4b])
        == PolymerConformation([c4b, c2, c3])
    )  # And order doesn't matter
    assert pc4 != PolymerConformation(
        [c2, c3, c4]
    )  # This should be different due to copying
...
def test_from_polymer_replacement():
    ...
    with pytest.raises(TypeError):
        PolymerConformation.from_polymer_replacement(
            pc1, [pc1.polymers[0]], None
        )
    ...
    with pytest.raises(ValueError):
        PolymerConformation.from_polymer_replacement(
            pc1, [pc1.polymers[0]], [p1, p2]
        )

    with pytest.raises(TypeError):
        PolymerConformation.from_polymer_replacement(
            None, [pc1.polymers[0]], [p1, p2]
        )
