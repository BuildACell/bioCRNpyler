...
def test_One_Step_Binding():
    ...
    assert c_fake in osb.update_species(
        binder, bindee, complex_species=c_fake
    )
...
def test_One_Step_Cooperative_Binding():
    ...
    assert c_fake in oscb.update_species(
        binder, bindee, cooperativity=1, complex_species=c_fake
    )
    # Try Cooperativity 2
    ...
    assert c_fake in oscb.update_species(
        binder, bindee, cooperativity=2, complex_species=c_fake
    )
    ...
    assert (
        len(
            oscb.update_reactions(
                binder, bindee, cooperativity=1, kb=1.0, ku=1.0
            )
        )
        == 1
    )
    ...
    assert (
        len(
            oscb.update_reactions(
                binder, bindee, cooperativity=2, kb=1.0, ku=1.0
            )
        )
        == 1
    )
...
def test_Two_Step_Cooperative_Binding():
    ...
    assert nmer_fake in tscb.update_species(
        binder, bindee, cooperativity=1, n_mer_species=nmer_fake
    )
    assert c1 in tscb.update_species(
        binder, bindee, cooperativity=1, n_mer_species=nmer_fake
    )
    assert (
        len(
    ...
    assert c_fake in tscb.update_species(
        binder, bindee, cooperativity=2, complex_species=c_fake
    )
    assert nmer_fake in tscb.update_species(
        binder, bindee, cooperativity=2, n_mer_species=nmer_fake
    )

    # Test Update Reactions
    assert (
        len(
            tscb.update_reactions(
                binder,
                bindee,
                cooperativity=1,
                kb=(1.0, 1.0),
    ...
    assert (
        len(
            tscb.update_reactions(
                binder, bindee, cooperativity=2, kb=(1.0, 1.0), ku=(1.0, 1.0)
            )
        )
        == 2
    )
