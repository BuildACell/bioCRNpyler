...
def test_CombinatorialConformation_init():
    ...
    pc1 = Complex(
        [pc0.polymers[0][0], pc0.polymers[0][2]]
    ...
    CC1b = CombinatorialConformation(
        initial_states=[pc0], final_states=[pc1b]
    )  # noqa: F841
    assert len(CC1.initial_states) == 1 and len(CC1.final_states) == 1

    pc1c = Complex([pc1b.polymers[0][0], pc1b.polymers[0][1]]).parent
    CC1c0 = CombinatorialConformation(
        initial_states=None, final_states=[pc1b, pc1c]
    )
    assert len(CC1c0.final_states) == 2
    assert len(CC1c0.initial_states) == 1
    CC1c1 = CombinatorialConformation(
        initial_states=[pc0, pc1b], final_states=[pc1c]
    )
    ...
    pc2 = Complex(
        [pc1.polymers[0][1], PolymerConformation(polymer=p2).polymers[0][1]]
    ).parent
    with pytest.raises(ValueError):  # Multiple internal polymers
        CC = CombinatorialConformation(
            initial_states=[pc1], final_states=[pc2]
        )

    with pytest.raises(ValueError):  # Multiple internal polymers
        CC = CombinatorialConformation(
            initial_states=[pc0, pc1], final_states=[pc2]
        )
    ...
    with pytest.raises(ValueError):  # Multiple internal polymers
        CC = CombinatorialConformation(
            initial_states=[pc0], final_states=[pc2], excluded_states=[pc1]
        )
    with pytest.raises(ValueError):  # Multiple internal polymers
        CC = CombinatorialConformation(
            initial_states=[pc0], final_states=[pc1], excluded_states=[pc2]
        )
    ...
    with pytest.raises(ValueError):
        CC = CombinatorialConformation(
            initial_states=[pc0], final_states=[pc1], intermediate_states=[C]
        )

    with pytest.raises(ValueError):
        CC = CombinatorialConformation(
            initial_states=[pc0], final_states=[pc1], excluded_states=[C]
        )  # noqa: F841
...
def pass_test_CombinatorialConformation_compute_complexes_to_add_to_polymer():
    ...
    assert len(complexes_to_add) == 2
    assert (0, [X]) in complexes_to_add and (
        2,
        [Complex([X, X])],
    ) in complexes_to_add
...
def test_compute_species_changes():
    ...
    assert (pc3.complexes[0], ind_c1) in SC and (
        pc3.complexes[1],
        ind_c3,
    ) in SC
    assert len(SC[pc3.complexes[0], ind_c1]) == 0
    assert (
        len(SC[pc3.complexes[1], ind_c3]) == 1
        and Z in SC[pc3.complexes[1], ind_c3]
    )
    assert all([len(MC[cf]) == 0 for cf in MC])

    # A single complex created, from a conformation with complexes
    SC, MC = CC.compute_species_changes(s0=pc1, sf=pc3)
    assert (pc3.complexes[0], ind_c1) not in SC and (
        pc3.complexes[1],
        ind_c3,
    ) in SC
    assert (
    ...
    assert (pc1.complexes[0], ind_c1) not in SC and (
        pc2.complexes[0],
        ind_c2,
    ) in SC
    assert (
        len(SC[pc2.complexes[0], ind_c2]) == 2
        and S in SC[pc2.complexes[0], ind_c2]
    )
    ...
    assert (
        Z in SC[pc4.complexes[0], ind_c4]
        and len(SC[pc4.complexes[0], ind_c4]) == 1
    )
    ...
    assert (
        (pc3.complexes[0], ind_c1) not in SC
        and (pc3.complexes[1], ind_c3) not in SC
        and (pc4.complexes[0], ind_c4) not in SC
    )
    assert (
        pc3.complexes[0] in MC[pc4.complexes[0], ind_c4]
        and pc3.complexes[1] in MC[pc4.complexes[0], ind_c4]
    )
...
def test_get_combinations_between():
    ...
    CC = CombinatorialConformation(
        initial_states=[], intermediate_states=[], final_states=[pc3]
    )
    ...
    CC = CombinatorialConformation(
        initial_states=[], excluded_states=[pc1], final_states=[pc3]
    )
    perms = CC.get_combinations_between(s0=pc0, sf=pc3)
    assert len(perms) == 2
...
def test_update_species_and_reactions():
    ...
    CC0 = CombinatorialConformation(
        initial_states=[], final_states=[pc0], parameters=params
    )
    ...
    CC1 = CombinatorialConformation(
        initial_states=[], final_states=[pc1], parameters=params
    )
    ...
    CC1b = CombinatorialConformation(
        initial_states=[pc0], final_states=[pc1], parameters=params
    )
    ...
    CC2 = CombinatorialConformation(
        initial_states=[pc0], final_states=[pc1, pc2], parameters=params
    )
    species = CC2.update_species()
    reactions = CC2.update_reactions()
    assert len(species) == 4
    assert (
        pc1 in species and pc0 in species and pc2 in species and S in species
    )
    assert len(reactions) == 2

    # Test multiple pathways
    CC3 = CombinatorialConformation(
        initial_states=[pc0], final_states=[pc3], parameters=params
    )
    species = CC3.update_species()
    reactions = CC3.update_reactions()
    assert len(species) == 5
    assert (
        pc1 in species and pc0 in species and pc3 in species and Z in species
    )
    assert len(reactions) == 4

    # Test intermediate states
    CC3b = CombinatorialConformation(
        initial_states=[pc0],
        intermediate_states=[pc1],
        final_states=[pc3],
        parameters=params,
    ...
    CC3c = CombinatorialConformation(
        initial_states=[pc0],
        excluded_states=[pc1],
        final_states=[pc3],
        parameters=params,
    ...
    CC4a = CombinatorialConformation(
    ...
    CC4b = CombinatorialConformation(
        initial_states=[pc0],
