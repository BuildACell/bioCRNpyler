...
def test_combinatorial_DNAconstruct_RNAconstruct():
    ...
    try:
        for combination in iter_list:
            x = DNA_construct(
                combination, mechanisms=mechs, parameters=parameters
            )
            print(x)
...
promoters = [
    ...
    RegulatedPromoter(
        'regulated_promoter', regulators=[Species('S1'), Species('S2')]
    ),
    CombinatorialPromoter(
        'combinatorial_promoter', regulators=[Species('S1'), Species('S2')]
    ),
]
                ...
                for t in terminators:
                    for rbs in RBSs:
                        for cds in CDSs:
                            ...
                            construct = m.get_component(construct)
                            crn = m.compile_crn()
                            inputs, outputs = all_reaction_inputs_and_outputs(
                                crn
                            )
