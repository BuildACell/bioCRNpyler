...
def Component_CRN_validation(CRN, component, mixture):
    ...
    if isinstance(component, DNAassembly):
        ...
        if isinstance(mixture, ExpressionExtract) or isinstance(
            mixture, ExpressionDilutionMixture
        ):
    ...
    if isinstance(component, ChemicalComplex):
        assert (
            component.get_species() in reaction_outputs
        )  # Complex can be formed
        assert all(
            [s in reaction_inputs for s in component.internal_species]
        )  # All species in the complex are inputs

    if isinstance(component, Enzyme):
        assert (
            component.enzyme in reaction_inputs
        )  # enzyme should be an input
        assert (
            component.enzyme in reaction_outputs
        )  # enzyme should be an output
        assert all(
            [s in reaction_inputs for s in component.substrates]
        )  # substrates should be inputs
        assert all(
            [s in reaction_outputs for s in component.products]
        )  # products should be outputs
...
class CombinatorialComponentMixtureTest(TestCase):
    def setUp(self) -> None:
        ...
        self.component_classes = [
            ...
            (
                DNAassembly,
                {'name': 'dna_assembly_v3', 'promoter': 'P', 'rbs': None},
            ),
            (
                DNAassembly,
                {'name': 'dna_assembly_v4', 'promoter': None, 'rbs': None},
            ),
            (
                DNAassembly,
                {
                    'name': 'dna_assembly_v5',
                    'promoter': 'P',
                    'rbs': 'rbs',
                    'transcript': None,
                    'protein': 'X',
        ...
        self.promoter_classes = [
            ...
            (
                ActivatablePromoter,
                {'name': 'activatable_promoter', 'activator': Species('A')},
            ),
            (
                RepressiblePromoter,
                {'name': 'repressible_promoter', 'repressor': Species('R')},
            ),
            (
                RegulatedPromoter,
                {
                    'name': 'regulated_promoter',
                    'regulators': [Species('R1'), Species('R2')],
                },
            ),
            (
                CombinatorialPromoter,
                {
                    'name': 'combinatorial_promoter',
                    'regulators': [Species('R1'), Species('R2')],
                },
            ),
        ]

        self.rbs_classes = [(RBS, {'name': 'rbs'})]
        ...
        self.mixture_classes = [
            ...
            (
                ...
                {'name': 'expression_dilution_mixture'},
            ),
            (
                SimpleTxTlDilutionMixture,
                {'name': 'simple_tx_tl_dilution_mixture'},
            ),
                ...
                for mixture, args in self.mixture_classes:
                    ...
                    if M.get_component(component=C) is None:
                        raise AttributeError(
                            f'{C} not found in M.components={M.components}'
                        )
                ...
                for rbs, args in self.rbs_classes:
                    ...
                    for mixture, args in self.mixture_classes:
                        ...
                        if M.get_component(component=A) is None:
                            raise AttributeError(
                                f'{A} not found in M.components={M.components}'
                            )
                        CRN = M.compile_crn()
                        document, _ = CRN.generate_sbml_model()
                        assert validate_sbml(document) == 0
                        ...
                        # Validate the CRN topology
                        Component_CRN_validation(
                            CRN=CRN, component=A, mixture=M
                        )
                        ...
                        if M2.get_component(component=A) is None:
                            raise AttributeError(
                                f'{A} not found in M.components={M2.components}'
                            )
                        ...
                        # Validate the CRN topology
                        Component_CRN_validation(
                            CRN=CRN2, component=A, mixture=M
                        )
                ...
                for rbs, args in self.rbs_classes:
                    ...
                    for mech_tx, args_tx in self.transcription_mechs:
                        for mech_tl, args_tl in self.translation_mechs:
                            ...
                            Mtl = mech_tl(**args_tl)
                            mechs = {
                                Mtx.mechanism_type: Mtx,
                                Mtl.mechanism_type: Mtl,
