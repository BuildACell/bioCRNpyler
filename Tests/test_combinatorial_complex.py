...
def test_CombinatorialComplex_init_and_properties():
    ...
    CC3 = CombinatorialComplex(
        final_states=[C1, C2], initial_states=[Complex([Z, Z]), X]
    )
    ...
    CC4 = CombinatorialComplex(
        final_states=[C1, C2], intermediate_states=[Complex([Z, Z])]
    )
    ...
    with pytest.raises(ValueError):
        CC = CombinatorialComplex(
            final_states=[C1], initial_states=[Species('S')]
        )
    ...
    with pytest.raises(ValueError):
        CC = CombinatorialComplex(
            final_states=[C1],
            initial_states=[Complex([Species('S'), Species('S2')])],
        )

    # intermiade_states must be Complexes
    with pytest.raises(ValueError):
        CC = CombinatorialComplex(
            final_states=[C1], intermediate_states=[Species('S')]
        )
    ...
    with pytest.raises(ValueError):
        CC = CombinatorialComplex(
            final_states=[C1],
            intermediate_states=[Complex([Species('S'), Species('S2')])],
        )  # noqa: F841
...
def test_CombinatorialComplex_compute_species_to_add():
    ...
    assert (
        species_to_add.count(X) == 1
        and species_to_add.count(Y) == 1
        and species_to_add.count(Z) == 1
    )
...
def test_CombinatorialComplex_get_combinations_between():
    ...
    assert (Y, X, Complex([X, Y])) in combos and (
        Z,
        X,
        Complex([X, Z]),
    ) in combos
    # Second steps are in combos
    assert (Z, Complex([X, Y]), Complex([X, Y, Z])) in combos and (
        Y,
        Complex([X, Z]),
        Complex([X, Y, Z]),
    ) in combos
    ...
    assert len(combos) == 4
    assert (X, X, Complex([X, X])) in combos and (
        Z,
        X,
        Complex([X, Z]),
    ) in combos
    assert (Z, Complex([X, X]), C2) in combos and (
        X,
        Complex([X, Z]),
        C2,
    ) in combos
...
def test_CombinatorialComplex_update_species():
    ...
    assert (
        Complex([X, Y]) in s1
        and Complex([X, Z]) in s1
        and Complex([Y, Z]) in s1
    )
    ...
    CC2 = CombinatorialComplex(
        final_states=[C2], initial_states=[C2i1, C2i2], mechanisms=[mech_b]
    )
    ...
    assert (
        Complex([X, Y, Z]) in s3
        and Complex([X, X]) in s3
        and Complex([X, Y]) in s3
    )
    ...
    CC4 = CombinatorialComplex(
        final_states=[C4],
        intermediate_states=[Complex([X, Y, Y]), Complex([X, Z, Z])],
        initial_states=[Complex([X, Y]), Complex([X, Z])],
        mechanisms=[mech_b],
    )
    ...
    assert (
        Complex([X, X]) in s5
        and Complex([X, Z]) in s5
        and Complex([X, Y]) in s5
    )
...
def test_CombinatorialComplex_update_reactions():
    ...
    def R(inputs, outputs):
        return Reaction.from_massaction(
            inputs, outputs, k_forward=kb, k_reverse=ku
        )
    ...
    CC1 = CombinatorialComplex(
        final_states=[CXYZ], mechanisms=[mech_b], parameters=params
    )
    ...
    CC4 = CombinatorialComplex(
        final_states=[CXXYYZZ],
        intermediate_states=[CXXYZ],
        initial_states=[CXX, CYZ],
        mechanisms=[mech_b],
        parameters=params,
    )
    ...
    # multiple final states
    CC5 = CombinatorialComplex(
        final_states=[CXXY, CXXZ], mechanisms=[mech_b], parameters=params
    )
