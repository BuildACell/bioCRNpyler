...
class reaction(object):
    def __init__(self, inputs, outputs, k, input_coefs = None, output_coefs = None, k_rev = 0, type = "massaction", propensity_params = None):
        ...
        elif type=="hillpositive" and ("s1" not in propensity_params or "K" not in propensity_params or "n" not in propensity_params):
        ...
        elif type=="hillnegative" and ("s1" not in propensity_params or "K" not in propensity_params or "n" not in propensity_params):
        ...
        elif type =="proportionalhillpositive" and ("s1" not in propensity_params or "d" not in propensity_params or "K" not in propensity_params or "n" not in propensity_params):
        ...
        elif type == "proportionalhillnegative" and ("s1" not in propensity_params or "d" not in propensity_params or "K" not in propensity_params or "n" not in propensity_params):
        ...
        elif type == "general" and "equation" not in propensity_params:
            raise ValueError("general propensities, p(s) = k * f(s), require the propensity_params: 'equation':f(s) where f(s) is an SBML compatable function of arbitrary species, s (use repr(chemical_reaction_network.specie) to get the proper text representation of a species name).")
    ...
    def __repr__(self, **kwargs):
        ...
        if self.type == "massaction":
            ...
            eq = self.propensity_params["equation"]
    ...
    def create_bioscrape_model(self):
        ...
        for rxn in self.reactions:
            ...
            if rxn.reversible and rxn.type == "massaction":
                ...
                #print("Adding Reverse Reaction", products, reactants, prop_type, prop_params)
                #model.create_reaction(products, reactants, prop_type, dict(prop_params))
            elif rxn.reversible:
                raise ValueError("Only massaction reversible reactions are supported for automatic bioscrape simulation. Consider creating two seperate reactions.")


        model = Model(species = species_list, reactions = reaction_list)
        return model

    def simulate_with_bioscrape(self, timepoints, initial_condition_dict, stochastic = False):
        import bioscrape
        m = self.create_bioscrape_model()
        #print("m.species_values", [(i, m.get_species_value(i)) for i in m.get_species_list()])
        m.set_species(initial_condition_dict)
        #print("m.species_values", [(i, m.get_species_value(i)) for i in m.get_species_list()])
        m._initialize()
        #print("m.species_values", [(i, m.get_species_value(i)) for i in m.get_species_list()])

        #print("get_species_list", m.get_species_list())
        #print("len(m.propensities)", len(m.get_propensities()))
        #print("m.propensities", m.get_propensities())
        #print("len(m.delays)", len(m.get_delays()))
