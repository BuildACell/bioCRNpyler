...
# Parameters
kb, ku, ktx, ktl, kdeg = 100, 10, 2.0, 3.0, 0.5
...
myMixture = BasicExtract(
    name='txtl', parameters=parameters, components=components
)
...
timepoints = np.arange(0, 3, 0.01)
stochastic = False

plt.figure(figsize=(16, 8))
plt.subplot(221)
plt.title('Load on a RNAP Promoter')
loads = [0, 1.0, 5.0, 10.0, 50, 100, 500, 1000]
for dna_Load in loads:
    ...
    results = myCRN.simulate_with_bioscrape(
        timepoints, x0_dict, stochastic=stochastic
    )
    plt.plot(
        timepoints, results['protein_ref'], label='Load = ' + str(dna_Load)
    )
...
for dna_Load in [0, 1.0, 5.0, 10.0, 25.0, 50, 100]:
...
for dna_Load in [0, 1.0, 2.0, 5.0, 10.0, 25.0, 50, 100]:
