...
rxn0_1 = Reaction.from_massaction(
    inputs=[G, A, A], outputs=[GA], k_forward=kb, k_reverse=ku
)
...
prop_hill_pos = ProportionalHillPositive(
    k=kex, s1=A, K=float(kb / ku), n=2, d=G
)
...
prop_hill_neg = ProportionalHillNegative(
    k=kex, s1=A, K=float(kb / ku), n=2, d=G
)
