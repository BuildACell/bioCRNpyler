...
r1 = Reaction.from_mass_action([s1], [s1, s2], k_forward=1.0)

#2 protein_R <--> dimer_R @ kf = 100, krev = 10
r2 = Reaction.from_mass_action([s2, s2], [s3], k_forward=100., k_reverse=10.) #Note duplicates in inputs
...
r3 = Reaction.from_mass_action(inputs=[s3], outputs=outputs, k_forward=100., k_reverse=10.)

#protein_R_deg --> NULL
r4 = Reaction.from_mass_action(inputs=[s4], outputs=[], k_forward=.1) #Empty set denoted as empty brackets
