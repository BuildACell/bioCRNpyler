...
kb_dcas_grna, ku_dcas_rgna = (
    100,
    0.01,
)  # binding constants for dCas9 and Guide RNAs
kb_dcas_dna, ku_dcas_dna = 100, 0.1
parameters = {
    'kb': kb,
    'ku': ku,
    'ktx': ktx,
    ...
    # default params
    ('dCas9_binding', 'kb'): kb_dcas_grna,
    ('dCas9_binding', 'ku'): ku_dcas_rgna,
    # binding constants for gRNA and dCas9
    ('dCas9_dna_binding', 'kb'): kb_dcas_dna,
    ('dCas9_dna_binding', 'ku'): ku_dcas_dna,
...
const_gRNA_assembly = DNAassembly(
    'gRNA', transcript=gRNA, promoter='P', rbs=None
)
...
parameters[('transcription', repressor.name, 'ktx')] = 0.1

# Create a Promoter regulated by the repressor
P_reg = RegulatedPromoter('P_regulated', regulators=[repressor], leak=True)
P_reg.default_mechanisms['binding'] = Reversible_Bimolecular_Binding(
    'dCas9_dna_binding'
)
...
reaction_mix = BasicExtract(
    'txtl', components=components, parameters=parameters
)
...
x0_no_dcas = {
    repr(const_dCas_assembly.dna): 0,
    repr(reg_assembly.dna): 1,
    repr(const_gRNA_assembly.dna): 10,
    'complex_Ribo': 20,
    'protein_RNAP': 10,
    'protein_RNAase': 10,
}
x0_with_dcas = {
    repr(const_dCas_assembly.dna): 2.0,
    repr(reg_assembly.dna): 1,
    repr(const_gRNA_assembly.dna): 10,
    'complex_Ribo': 20,
    'protein_RNAP': 10,
